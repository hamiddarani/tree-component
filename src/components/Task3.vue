  ...
  <div>
      <ul>
          <li v-for="child in children" :key="child.id">
              <b-form-checkbox :checked="checked(child)" :indeterminate="intederminateCondition(child)" @change="change(child)">
                  {{child.id}}
              </b-form-checkbox>
    ...
    methods : {
        ...
        change(child){
            if(this.value.ids.includes(child.id)){
                if(child.parentId !== null){
                    this.forwardLoopUnselect(this.simpleData.find(item => item.id === child.parentId))
                }
            ...
            if (this.reverseLoopUnselectCondition(child.parentId)) {
                if(child.parentId !== null){
                    ...
                    this.reverseLoopSelect(this.simpleData.find(item => item.id === child.parentId)?.parentId)
                }
            }
        ...
        reverseLoopSelect(id){
            if (this.reverseLoopUnselectCondition(id) && id !== null) {
                ...
                this.reverseLoopSelect(this.simpleData.find(item => item.id === id)?.parentId)
            } 
        },
        filterArray(id){
        ...
        reverseLoopUnselectCondition(id){
            return this.filterArray(this.simpleData.find(item => item.id === id)?.id).every(item => {
